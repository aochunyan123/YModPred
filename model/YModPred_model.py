...
from .pooling_method import MeanPooling,MaxPooling,MinPooling,AttentionPooling
...
class YModPred(nn.Module):
    def __init__(self):
        super(YModPred, self).__init__()
...
#             nn.Dropout(0.5),
    ...
    def forward(self, input_ids):
        ...
        for layer in self.layers:
            enc_output = layer(cnn_output, enc_self_attn_mask)

        enc_seq = enc_output[:,1:,:]
        pooled_output = self.pool(enc_seq, attention_mask[:,1:])

        logits = self.block1(pooled_output)
        out2 = F.elu(self.block2(pooled_output))
        out1 = F.softmax(logits, dim=-1)
